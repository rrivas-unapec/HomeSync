...
- Ejecutar `HomeSync.sql` contra tu instancia de SQL Server si aún no lo hiciste.
- Ajustar CORS en `Program.cs` (`CorsOrigenesPermitidos` en `appsettings.json`) con la URL real de tu frontend cuando lo despliegues.
- Escribir la colección de Postman/Insomnia o pruebas automatizadas que pide el enunciado (puedo ayudarte a generarla).
- Desplegar en Render/Railway/Azure y documentar la URL pública en el README del repo final.

No pude compilar esta solución con `dotnet build` completo en este entorno porque
no tengo acceso a `api.nuget.org` para restaurar paquetes (EF Core, JWT,
Swashbuckle, BCrypt). Sí compilé y verifiqué `UrbanSync.Domain` (sin
dependencias externas) sin errores. Te recomiendo correr `dotnet restore` y
`dotnet build` en tu máquina antes de seguir avanzando, por si aparece algún
detalle de compilación que aquí no pude ver.
